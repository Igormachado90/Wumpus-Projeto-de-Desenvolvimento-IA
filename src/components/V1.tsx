    ...
    <main className="validacao-tela">
      ...
      {resultado && (
        <>
          ...
          <section className="validacao-graficos painel">
            <h2>V3 — Evolução do fitness por geração (um gráfico por tamanho de ambiente)</h2>
            <div className="grade-geracoes">
              {tamanhos.map((t) => (
                <div key={t} className="subplot-geracao">
                  <h3 className="subtitulo-grafico">n = {t}</h3>
                  <GraficoLinhaGeracoes dados={curvaMediaV3PorTamanho(resultado.curvasV3, t)} compacto />
                </div>
              ))}
            </div>
          </section>
      ...
      <g transform={`translate(${margemEsq}, 6)`}>
        {versoes.map((v, i) => (
          <g key={v} transform={`translate(${i * 150}, 0)`}>
            <rect width="10" height="10" fill={CORES[v]} rx={2} />
...
function GraficoLinhaMetrica({
  ...
  return (
    <svg viewBox={`0 0 ${largura} ${altura}`} className="grafico-svg" role="img" aria-label="Gráfico de linha por versão">
      ...
      {versoes.map((v) => {
        const pontos = tamanhos.map((t, i) => {
          const item = resumo.find((r) => r.tamanho === t && r.versao === v);
          const valor = item ? acessor(item) : 0;
          return { x: xEsc(i), y: yEsc(valor), valor };
        });
        ...
        return (
          <g key={v}>
            ...
            {pontos.map((p, i) => (
              <circle key={i} cx={p.x} cy={p.y} r={3} fill={CORES[v]}>
                <title>{`${NOMES[v]} · n=${tamanhos[i]}: ${formato(p.valor)}`}</title>
              </circle>
            ))}
          </g>
        );
...
function GraficoLinhaGeracoes({
  ...
  const largura = compacto ? 320 : 640;
  const altura = compacto ? 180 : 280;
